...
//                }
    ...
    private fun decodeMessage(wrap: ByteBuffer): WatchCommunicationRawResponse {
        val buf = ByteBuffer.wrap(decryptMessage(wrap)).order(ByteOrder.BIG_ENDIAN)
        val sn = buf.int // sn generated by watch
        val ackSn = buf.int // That's the sn of the package that we had sent to the watch
        val command = buf.short // cmd
        val length = buf.short.toInt() // length
        if (buf.hasRemaining()) {
            ...
            return WatchCommunicationRawResponse(
                sn, ackSn, command, rawContents
            )
        }
        // FIXME log here, or throw exception?
        return WatchCommunicationRawResponse(
            sn, ackSn, command, ByteArray(0)
        )
    }

    private fun decode3(wrap: ByteBuffer): WatchCommunicationRawResponse {
        // FIXME check
    ...
    private var listeners = HashSet<WatchCommunicatorListener>()
    ...
    private fun decodeBigMessage(wrap: ByteBuffer): WatchCommunicationRawResponse {
        val buf = ByteBuffer.wrap(decryptMessage(wrap)).order(ByteOrder.BIG_ENDIAN)
        val sn = buf.int // sn generated by watch
        buf.get() // TODO
        val command = buf.short
    ...
    fun addListener(listener: WatchCommunicatorListener): WatchCommunicator {
